...
### Startup UI (superseded 2026-06-20 — see BootScene below)
...
### BootScene / StartMenu (current, 2026-06-20)

`BootScene` is the normal entry scene (build index 0), with a real, scene-/prefab-authored Canvas — not generated by code.
...
- `TestCombat` opened directly in the editor (no `BootScene`) still falls back safely to an unselected-character fresh state.
...
### Player Input (updated 2026-06-20 — persistent target model)

LMB hold over a WorldDrop (Drop layer) → collect drop. This check runs before all other input.

Single LMB click — priority order unchanged: drop pickup → UI (`EventSystem.IsPointerOverGameObject`) → enemy → `WorldInteractable` → plain ground move.

**Enemy click and AutoCombat now share one persistent `_currentTarget` + `Moving`→`Attacking` loop** (`PlayerCombatController`) — the old one-shot `ManualAttack`/`_manualAttackTarget` path was removed:
...
- `MapSystem` — kill tracking, travel, objective completion. Note: `PortalGate`/`MapContentController` read/write `MapProgressData.IsUnlocked` via `GetProgress()` only — `MapSystem.cs` itself is untouched (see "Quest/Portal/Map Architecture" below).
- `PlayerStats.Recalculate()` — stat pipeline (only extend at the end with new bonuses)
- `ItemWindow`, `CraftingWindow`, `VaultWindow`, `MapWindow` — window UI logic (buttons and events are wired). `CraftingWindow`/`CraftingDetailPanel`/`CraftingRecipeSlotUI`/`IngredientRowUI` were intentionally modified 2026-06-18 under explicit instruction to adapt to a manual UI redesign — still protected going forward.
- `ObjectiveHelperUI` — disabled (not deleted) in-scene for the quest demo (see below); still protected if re-enabled.
...
## Session Update — Dialogue, QuestSystem Phase A, Gates (2026-06-19)

Committed through HEAD `ee10615`. See `.claude/HANDOFF_CURRENT_STATE.md` for the full current-state map.

- **Dialogue wired end-to-end.** Headless `IdleOn.Dialogue` runtime + `DialogueWindowUI` (on Canvas, starts hidden, full-screen click → `Advance()`) + `NPCDialogueInteractable` + scene `DialogueSystem` host. `DialogueSystem` raises `GameEvents.OnDialogueEnded(dialogueId)` once on full end. Branching/`Choose()` kept but unused (linear demo). Test NPC at x=2 (`DialogueDef_TestChief`, id `test_chief`).
- **QuestSystem Phase A (in-memory, no save).** `_scripts/Quests/`: `QuestSystem` (singleton, one active quest, linear `NextQuestId`, `IsCompleted`), `QuestDefinition`/`QuestObjective`/`QuestProgress`/`QuestDatabase` (db via `[SerializeField]`, not GameDatabase), `FeatureUnlockSystem` + `[Flags] FeatureFlags{AutoCombat,Craft,Talents,Vault,Map}`. Q1–Q5 author'd (q1 ground-move, q2 map `grassland_2`, q3 kill `slime`, q4 map `town`, q5 dialogue `chief_q5`), q6 = inert placeholder. Rewards via new `PlayerProgression.AwardExp`. Map/portal unlocks are Debug.Log placeholders.
...
## Session Update — Q6–Q10, Multi-Map Rework, Portal Redesign (2026-06-19, uncommitted)

**Working tree is uncommitted at time of writing — see `.claude/HANDOFF_CURRENT_STATE.md` for the exact file list and a manual test route.**

### Quest/Portal/Map Architecture

The unlock model changed: **the destination `MapDefinition` is now the sole source of truth for what unlocks a portal**, not the portal itself and not `QuestSystem`/`MapSystem`.
...
### Multi-map single-scene rework

`TestCombat` now holds one player + 4 map-root GameObjects (`Map_grassland_1/2`, `Map_town`, `Map_grassland_3`) instead of one flat map. New `MapContentController` (`_scripts/World/`) listens for `OnMapChanged`, activates only the current root, and moves the persistent Player to that map's configured spawn — still data-only travel, no scene loads, per the existing "Do NOT add a WorldMap scene" rule. Global `EnemySpawner` is disabled for this slice; each map root has its enemies pre-placed.

### Q6–Q10 content

All four previously-deferred `GameEvents` raisers are now wired: `DropManager.Collect`→`RaiseItemCollected`, `CraftingSystem.Craft`→`RaiseItemCrafted`, `EquipmentSystem.Equip`→`RaiseItemEquipped`, `TalentSystem.Upgrade`→`RaiseTalentUpgraded`.

`QuestDef_Q6`..`QuestDef_Q10` authored (objectives/rewards table in `.claude/HANDOFF_CURRENT_STATE.md`). New `QuestDialogueSelector` (`_scripts/Quests/`) lets one Chief NPC serve different dialogues (`DialogueDef_ChiefQ5/Q6/Q8/Q10`) keyed off `QuestSystem.ActiveQuestId` — `NPCDialogueInteractable` asks it first, falls back to its fixed dialogue if no match. New items: `ItemDef_SlimeSword` (equipment), `ItemDef_SlimeEssence` (material), `Recipe_SlimeSword` (crafting).
